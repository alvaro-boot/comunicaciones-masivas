export const RESERVED_KEYS = new Set(["nombre", "usuario", "telefono", "saludo"]);
...
export function safeFieldKey(label, existingKey) {
  let key = fieldKey(existingKey || label);
  if (RESERVED_KEYS.has(key)) key = `campo_${key}`;
  return key;
}
